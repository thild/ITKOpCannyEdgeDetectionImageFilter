...
void
StopWatch::Checkpoint(const string step)
{
    double start = _start;
    _mesuringSteps.push_back(MeasuringStep(step, this->GetElapsedTime() - start));
}
...
vector<MeasuringStep>& StopWatch::GetMeasuringSteps()
{
  return _mesuringSteps;
}


//
//
//inline std::ostream& operator<<(std::ostream& os, MeasuringStep& t)
//{
//  os <<  std::setprecision(4) << std::setiosflags(std::ios::fixed)
//    << "Step: " << t.Step << "; Time: " << t.Time;
//  return os;
//}
